// Package envmigrate implements the parsing/merge logic behind `webmanager
// --env-migrate` (see webmanager/.claude/env-migration-plan.md for the full
// design). It reconciles a user's existing .env.webmanager against the
// image's current example-env.webmanager template: keys the user actively
// uncommented are kept, comments the user wrote themselves are preserved,
// keys the template no longer has are archived instead of dropped, and the
// template's own key set/order/section structure always wins (so added or
// renamed keys show up automatically).
package envmigrate
...
// OldEntry is a key's state as found in the user's existing .env.webmanager.
...
// Old is the fully parsed state of a user's existing .env.webmanager.
type Old struct {
	...
	EntryOrder []string
	Dead       []DeadEntry
	// Version is WEBMANAGER_ENV_VERSION's old active value, or "" if unset/
	// inactive (e.g. a file that predates this feature entirely).
	Version string
	Notes   []Note
}

func parseOld(content string) Old {
	...
	version := ""
	if e, ok := entries["WEBMANAGER_ENV_VERSION"]; ok && e.Active {
		version = e.Value
	}
...
// ParseVersion extracts WEBMANAGER_ENV_VERSION's active value from an
// env-file-shaped document (used on both example-env.webmanager, to get the
// image's current version, and — in principle — any other such file).
// Returns "" if unset/inactive.
func ParseVersion(content string) string {
	for _, line := range strings.Split(content, "\n") {
		c := classifyLine(line)
		if c.kind == kindValue && c.active && c.key == "WEBMANAGER_ENV_VERSION" {
			return c.value
		}
	}
	return ""
...
// Migrate reconstructs a .env.webmanager from old (the user's existing file,
// possibly empty for a fresh install) against template (the image's current
// example-env.webmanager). See the package doc and
// webmanager/.claude/env-migration-plan.md for the full behavior.
func Migrate(old, template string) Result {
	...
	templateVersion := ""
	for _, c := range tCls {
		if c.kind == kindValue && c.key == "WEBMANAGER_ENV_VERSION" {
	...
	for i := 0; i < len(tCls); i++ {
		...
		case kindValue:
			...
			case flagged[i] && hasOld && oe.Active:
				...
				out = append(out,
					fmt.Sprintf("#! .env.webmanager 버전 %s → %s 마이그레이션 중 이 키의 권장 기본값이", fromV, templateVersion),
