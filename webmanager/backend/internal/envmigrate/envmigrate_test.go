...
func TestMigrate_ForcedKeyOnFreshInstallStillNotes(t *testing.T) {
	template := "#!important\nWEBMANAGER_ENV_VERSION=2\n"
	old := ""

	res := Migrate(old, template)
	if res.Output != "WEBMANAGER_ENV_VERSION=2\n" {
...
func TestMigrate_FlaggedKeyKeepsUserValueAndShowsAdvisory(t *testing.T) {
	template := "#!important\nWEBMANAGER_ENV_VERSION=3\n\n#. desc\n#!\n#KEY_E=newdefault\n"
	old := "WEBMANAGER_ENV_VERSION=2\nKEY_E=oldvalue\n"
...
func TestParseVersion(t *testing.T) {
	if v := ParseVersion("#!important\nWEBMANAGER_ENV_VERSION=5\n"); v != "5" {
		t.Errorf("got %q, want %q", v, "5")
	}
	if v := ParseVersion("#WEBMANAGER_ENV_VERSION=5\n"); v != "" {
