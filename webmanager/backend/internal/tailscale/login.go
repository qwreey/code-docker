...
// demand, so webmanager can offer a "로그인 시도하기" retry button once
...
// control server in the first place).
//
// Unlike claudecode.LoginManager, no stdout scraping or stdin relay is
// needed: `tailscale status --json` (GetStatus above) already exposes
// BackendState/AuthURL structurally, so the frontend just re-polls the
// existing status endpoint after calling Start. That makes this manager
// much thinner - it only needs to track whether a process is currently in
// flight, to avoid starting a second concurrent `tailscale up` against the
// same daemon.
...
// no-op when nothing is running, matching claudecode.LoginManager.Cancel's
// contract so the frontend can call it best-effort without error handling.
