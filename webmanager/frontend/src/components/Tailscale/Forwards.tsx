...
export function Forwards() {
  ...
  const load = useCallback(async () => {
    try {
      const data = await api.get<TailscaleForward[]>('/tailscale/forwards')
      setForwards(data)
      setError(null)
    } catch (e) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setFormError(null)
    try {
      await api.post('/tailscale/forwards', {
        name,
  ...
  async function handleDelete(forwardName: string) {
    ...
    try {
      await api.del(`/tailscale/forwards/${encodeURIComponent(forwardName)}`)
      await load()
      showNotice()
    } catch (e) {
      await load()
