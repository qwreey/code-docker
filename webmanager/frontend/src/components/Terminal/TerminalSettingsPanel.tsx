    ...
    <Sheet open={open} onClose={onClose} title="터미널 설정">
      ...
      <section className="terminal-settings-section">
        ...
        <p className="section-description">
          ...
          기본으로 켜져 있습니다(갤럭시에서 깜빡임 없음 확인). 다만 기기에 따라 <strong>띄어쓰기마다 키보드가 깜빡
          닫혔다 열릴 수 있어</strong>, 그런 기기라면 끄세요(입력 디버그에 <code>(refocus: ...)</code>가 찍힙니다).
        </p>
        <label className="checkbox-option">
          <input
            type="checkbox"
        ...
        <p className="section-description">
          입력·화면 크기(키보드 열고 닫기)·관성 스크롤이 이상하면 이걸 켜고 문제가 나는 상황을 한 번 재현한 뒤, 오버레이 오른쪽 위의
          <strong> 복사</strong> 버튼을 누르세요. 어떤 이벤트가 어떤 값으로 왔고 실제로 어떤 바이트를 보냈는지가
          기기/입력 방식/브라우저 정보와 함께 텍스트로 클립보드에 들어갑니다(화면에는 최근 12줄만 보이지만
          복사되는 건 최근 400줄입니다). <strong>지우기</strong>로 비운 뒤 재현하면 더 깔끔합니다.
        </p>
        <p className="section-description">
          터치로 스크롤한 뒤 손을 떼면, 놓는 순간의 속도로 계속 스크롤되다가 서서히 멈춥니다(Termux나 일반적인
          안드로이드 목록과 같은 동작). 다시 화면을 터치하면 그 자리에서 멈춥니다. 스크롤백 끝에 닿으면 남은
          속도를 흘리지 않고 바로 멈춥니다. vim(<code>mouse=a</code>)이나 tmux처럼 앱이 마우스를
          켠 경우에는 관성이 휠 스크롤로 앱에 계속 전달됩니다(데스크탑 트랙패드의 관성 스크롤과 같은 동작). 반대로
          마우스를 켜지 않은 전체 화면 앱에는 <strong>일부러 관성이 붙지 않습니다</strong> — 그쪽은 스크롤 한 칸이
          방향키 입력이라, 손을 뗀 뒤에도 방향키가 수십 번 더 들어가면 곤란하기
          때문입니다.
        </p>
        <label className="checkbox-option">
          <input
