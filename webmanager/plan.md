...
| 비밀번호 게이트 지문(WebAuthn) 잠금 해제 — 서버 측 WebAuthn(`internal/webauthnunlock`, go-webauthn), 비밀번호로 푼 직후 등록 제안/사이드바 지문 아이콘에서 기기 관리, 잠금 해제 창의 지문 버튼(클릭으로 열린 창이면 자동 1회 시도, 취소 시 비밀번호로 폴백 + 재시도), 호스트별 등록, 사용자 확인 필수, 12시간 상한은 마지막 비밀번호 입력 기준 유지, 비밀번호 해시 변경 시 전부 해제, `WEBMANAGER_WEBAUTHN_ENABLED`로 끔 | `.claude/qa-request/webauthn-unlock-done.md` (가상 인증기로 e2e 확인, 실기기·passkeyd 확인 대기) |
