...
| File share(WebDAV) 탭 — 파일 매니저와 같은 트리를 `golang.org/x/net/webdav`로 내보내 다른 기기에서 네이티브 마운트(`internal/webdavshare`). 기본 꺼짐 + fail-closed(비밀번호 없으면 404), webmanager 게이트와 **분리된** Basic auth 비밀번호(WebDAV 클라이언트가 SSO 리다이렉트를 못 타서 바깥 forward-auth에서 빼야 하므로 강제된 설계), argon2id 검증 5분 캐시(클라이언트가 매 요청 재인증하므로 없으면 못 씀), 심볼릭 링크 탈출은 `internal/files`와 공유, 전용 nginx 리스너(`NGINX_WEBDAV_PORT`, 기본 82)로 router vhost가 전용 호스트네임을 줄 수 있음 | `.claude/archive/webdav-file-share-plan-done.md` (레포 루트), 사용자 문서 `docs/tips/webdav.md` |
...
| 바인드 주소 전략 확정(구 TODO 5번) — code-server/webmanager를 loopback(`127.0.0.1`) 대신 전용 tailscale IP(`private` 호스트네임)에 바인드, nginx `listen 127.0.0.1:80`으로 tailscale 자동 loopback 포워딩 경로 차단, `ALLOWED_HOSTS`(nginx Host 헤더 화이트리스트) 추가 | `router/docs/router.md`의 "보안" 절 (당시엔 레포 루트 `docs/tailscale.md`에 있었으나, tailscale이 router로 이관되며 `router/docs/`로 옮겨짐 — 전용 webmanager 문서는 없음, nginx/code-server/docker-compose.yml 전체에 걸친 변경이라 레포 루트 문서가 소관) |
...
| 폰트 매니저(신규, `internal/fonts`) — ttf/otf/woff/woff2 업로드+family별 아코디언 목록/편집/삭제(`GET/POST/PATCH/DELETE /api/fonts*`, 읽기·CSS·파일 서빙은 게이트 없음/쓰기만 게이트), `GET /api/fonts/css`가 매 요청 생성하는 `@font-face` 정의를 webmanager 자신의 `index.html`이 `<link>`로, code-server는 새 code-patch `config/code/code-patch/fonts.default.css`(내용이 절대 안 바뀌는 한 줄짜리 `@import`)로 각각 로드 — code-server 쪽은 강제 적용 없이 `editor.fontFamily`/`terminal.integrated.fontFamily`에 이름을 직접 넣으면 브라우저가 알아서 매칭(Fonts 탭에 안내문+복사 버튼). webmanager 자신의 웹터미널만 `internal/terminalsettings.Settings.FontFamily`로 선택 폰트를 실제 xterm 옵션에 반영. Victor Mono/BlexMono Nerd Font Mono(Mono 배리언트)/kuskhan-jetendard 전부 `recommendations.default.yaml`의 `fonts:` 키 엔트리로 통합, Extensions/mise 탭과 동일한 "추천 목록에서 클릭 설치" 방식(`POST /api/fonts/install {id}`, 다운로드 자체가 짧아 백그라운드 job 없이 동기 처리, 설치 여부는 family 이름 매칭으로 판단). jetendard만 `main.defaultFontSeedIDs`(추천 목록 id만 참조, URL 중복 없음)로 컨테이너 첫 부팅 시 추가 자동 설치되고(`SEED_DEFAULT_FONTS`로 opt-out), 이후 삭제/재설치는 다른 추천 폰트와 동일하게 사용자 몫(2026-08-18) | `.claude/archive/font-manager-plan-done.md` (레포 루트) |
...
| Claude 탭 관리 서브탭에 settings.json 원본 편집(`GET`/`PUT /api/claude/settings`, JSON 검증 후 쓰기, `ClaudeSettings.tsx`가 raw CodeMirror 에디터와 `autoCompactEnabled` 토글이 fetch 하나를 공유하게 해서 `SshConfigRaw.tsx`/`RawConfigEditor.tsx`가 겪었던 stale-fetch-on-mount 데이터 손실 버그를 피함)와 로그아웃 버튼(`POST /api/claude/logout`, `claude auth logout` 실행) 추가, 둘 다 쓰기라 게이트 | 문서 없음(2026-08-10, 작은 기능 추가) |
| Terminal 세션 ↔ Projects 연동 — 세션의 실시간 cwd를 Terminal 홈/탭 툴팁에 표시하고 거기서 해당 프로젝트로 점프, 반대로 Projects 상세 시트에 그 프로젝트 아래 열린 터미널 세션 목록 표시(`App.tsx`의 기존 `pendingTerminalOpen`/`pendingFilesPath` 탭 간 payload 메커니즘 재사용) | 문서 없음(2026-08-10, 작은 기능 추가) |
| Logs 탭에 앱별 vector 로그 purge(`DELETE /api/logs?app=<name>`, 앱별 날짜 파일 JSONL을 다시 써서 해당 앱 라인만 제거, 매치 없는 날짜 파일은 그대로 둠; `?date=`는 날짜 파일 전체 삭제용으로 API에만 있고 프론트 UI는 없음) — 다른 파괴적 동작과 동일하게 게이트+확인 다이얼로그 | 문서 없음(2026-08-10, 작은 기능 추가) |
| `webmanager --attach <name> [start-dir]`(`bin/attach`로 래핑, PATH 어디서나) — SSH/code-server 터미널/webmanager 터미널 탭 안에서 **이미 열려있는 webmanager Terminal 탭 세션에 그대로** 붙는 CLI. 브라우저가 쓰는 것과 완전히 같은 `GET /api/terminal?session=` WebSocket을 그대로 사용, 별도 세션/tmux 전혀 없음 — `internal/termsession.Session`을 진짜 동시-다중-attach(broadcast)로 바꾸고, 재접속 시 링버퍼 재생 앞에 화면 clear를 붙임. 종료 시퀀스는 기본 Ctrl+](Ctrl+P Ctrl+Q는 code-server 자체 단축키와 겹쳐서 기본에서 제외, 프리셋으론 유지), 터미널 설정 패널에서 직접 편집 가능. 이미 세션 안에 있으면(중첩) 이름 상관없이 attach 자체를 거부(순환 중첩 방지). tmux 두 번 시도했다가 전부 되돌림 — 자세한 경위/실사람 QA 체크리스트는 계획 문서 참고 | `.claude/qa-request/attach-cli-plan-done.md` |
| 웹터미널 확대/축소 — 특수키 바 가장 오른쪽에 아이콘 버튼(`--accent` 톤으로 구분), localStorage에 기기별 저장(8~32px 클램프), 확대/축소 시 `fitAddon.fit()` + resize 메시지까지 처리 | 문서 없음(작은 기능, 실브라우저에서 직접 확인 완료 — 확대/축소/새로고침후 유지/클램프/버튼 위치·색 전부 확인) |
| 사용자 요청 배치(6건) — 데스크탑 사이드바 접기(사이드바 제목 옆 « 버튼, 접으면 상단에 얇은 메뉴바로 남음, `margin-left` 슬라이드 방식이라 width 애니메이션과 달리 내부 항목이 리플로우/찌그러짐 없음, localStorage에 상태 저장); code-server 위젯의 웹매니저 열기 아이콘이 미들클릭/Ctrl+클릭 시 새 탭으로 열리도록(`config/code/code-patch/webmanager-launcher.default.js`, 기존엔 진짜 링크가 아니라 클릭 리스너뿐이라 아예 무반응이었음); 열린 세션 탭에 브라우저(기기) 단위 그룹화 — code-patch가 탭별 UUID 옆에 localStorage 기반 browserId도 같이 heartbeat로 보내고, 신규 `internal/browsernames`(파일 영속화)로 그룹별 사용자 지정 별칭 CRUD(`GET/PUT /api/sessions/browsers*`); 터미널 연결 끊김 시 재연결 다이얼로그(터미널 영역에만 스코프된 dim 오버레이 + 재연결 버튼, 사이드바/다른 탭 이동은 그대로 자유로움); 터미널 탭 자체에 OSC 52 클립보드 핸들러 신규 추가(git 이력상 원래 없었음 — 있던 건 Claude 로그인 다이얼로그의 별도 PTY뿐, 회귀 아님); 모바일 가상 키보드 예측입력 버퍼링 우회(실험적, 터치 기기에서만 실제 `<input type="password">`를 만들어 포커스를 가로채고 sentinel 문자+길이 diff 방식으로 백스페이스까지 처리, 터미널 설정 패널에서 끄고 켤 수 있음/localStorage) — 실기기 확인상 버퍼링·자동완성 팝업 문제는 해결됐지만 빠른 연속 입력 시 속도 저하가 남아있음, 후속 디버깅은 레포 루트 `TODO.md` 참고 | 문서 없음(2026-08-19, 사용자가 직접 제보한 이슈 6건을 한 세션에서 순차 처리) |
| 사용자 노트 배치(8건, 2026-09-06) — 터미널 모바일 입력 방식 3종 선택(`native`/`password`/**`diff`**, 기본 `diff`: 숨겨진 `<textarea>`에 입력을 쌓아두고 이미 보낸 문자열과의 공통 접두사 diff만 전송 → 삼성 키보드의 중복 입력과 한글 자모 분리를 동시에 겨냥, 모드 변경은 탭 재마운트 없이 즉시 적용) + 입력 디버그 오버레이; 하단 컨트롤 바 버튼이 키보드를 **열지는 않고 유지만** 하도록 `focusTerminal` → `keepFocus`; 터치 모드 3종(스크롤/마우스/선택 — `select`는 합성 마우스 이벤트에 `shiftKey`를 실어 xterm의 force-selection을 쓰는 것)과 선택 텍스트 복사(평문 http용 `execCommand` 폴백 포함 `utils/clipboard.ts` 공용화); 터미널 탭에서 앱 상단 바를 숨기고 햄버거를 터미널 헤더로 흡수(세로 3.6rem 확보); 탭 안 상태를 URL로(`?session=`/`?path=`/`?project=`, `replaceState`) — 죽은 세션 이름은 **새로 만들지 않고** 홈으로; 파일 브라우저를 현재 탭 위 전체화면 오버레이로(`FileManagerDialog`); 파일 에디터 전체화면 + 줄바꿈 토글(`Sheet`에 `size="full"`, `CodeEditor`에 `wrap`); code-server 위젯의 "새 탭으로 열기" | `.claude/archive/terminal-mobile-input-touch-plan-done.md`, `.claude/qa-request/url-state-and-file-overlay-plan-done.md` |
| code-server 위젯에 최대화 버튼(닫기 옆, localStorage에 상태 저장, `config/code/code-patch/webmanager-launcher.default.js`); router 비밀번호 설정 화면의 `.info-note` 간격 버그(`margin-bottom`만 있어서 위 폼과 붙어보이던 문제, `margin: 1rem 0`으로 수정); SSH 기본 키 빈 상태의 "키 생성" 버튼이 안내문과 붙어보이던 간격 버그 수정(`GitLFS.tsx`와 동일한 flex-column 래퍼 패턴); 터미널 탭에서 모바일 키보드가 열릴 때 페이지 전체가 스크롤되던 버그(`html`/`body`/`#root`에 `height:100%; overflow:hidden` 안전망 추가); 터미널 세션의 cwd가 `cd` 직후 즉시 반영 안 되던 버그(백엔드는 `/proc/<pid>/cwd`를 라이브로 읽지만 프론트가 connect/disconnect/rename 시에만 재조회했음 — 실은 폴링이 처음부터 구현된 적이 없었던 미구현 기능, `Terminal.tsx`/`ProjectTerminalSessions.tsx`에 3초 폴링 추가); 720px 이하에서 터미널 탑바(연결 상태/파일 브라우저/프로젝트로 이동/설정)가 텍스트로 줄바꿈되던 문제를 아이콘 전용으로 축소(설정 버튼엔 원래 아이콘이 없어서 신규 추가) | 문서 없음(2026-08-19, 사용자 제보 4건+라이브 후속 1건, 커밋 `fa382d9`..`cc87a52`) |
| 사용자 요청 배치(7건) — 비밀번호 게이트 슬라이딩 만료(`internal/authgate`: 토큰 payload가 `origin:refreshed` 두 값이 되고 `RequirePassword`가 검증 성공 시 재발급, 유휴 10분/절대 12시간 상한, `:` 없는 구버전 토큰도 그대로 수용 — 터미널 탭의 3초 폴링이 만료 순간부터 401을 계속 뱉어 비밀번호 모달이 3초마다 다시 뜨던 것이 실제 증상, router-manager 게이트도 동일 수정) + 모달을 취소하면 60초간 읽기 요청발 재프롬프트 억제(`api/client.ts`); 터미널 재접속 렌더 깨짐(스크롤백은 파싱 안 된 raw 바이트인데 클라이언트 `resize` 제어 메시지가 리플레이 *뒤에* 읽혀서 옛 크기 기준 이스케이프가 새 화면에 그려짐 — WS 접속 URL에 `cols`/`rows`를 실어 `Attach()` 전에 리사이즈하고, 같은 크기 재접속은 TIOCSWINSZ가 SIGWINCH를 안 보내므로 리플레이 직후 한 줄 줄였다 되돌리는 `nudgeRepaint`로 전체 리페인트 유도, `attach` CLI도 동일; 숨겨진 컨테이너에서 `fit()`이 2×1로 클램프되던 것도 `fitIfVisible()`로 차단); 세션 기록 각 행에 "터미널에서 이어하기"(`claude --resume <id>` + 그 대화의 cwd, `openInTerminal`에 `command` 인자 추가); 터미널의 "프로젝트로 이동"을 탭 이동 대신 그 자리 다이얼로그로(`ProjectInfoDialog` — Projects 상세 시트와 같은 path-only 패널들을 재사용, 관리 동작은 "Projects 탭에서 열기"로 넘김); 데스크탑 사이드바 접기가 상단 메뉴바(≈48px 세로 점유)를 띄우던 것을 좌측 16px 세로 스트립(`.sidebar-rail`, 호버 시 20px)으로 교체 — 데스크탑은 세로가 더 아쉬운 축; Projects 상세 시트에서 "이 프로젝트에서 열린 세션"을 개요 바로 아래로 올리고 기본 펼침; 접기 핸들 디자인 통일(`components/common/CollapseChevron.tsx` 하나로, 텍스트 `▶`/lucide 쌍 등 7개 파일에 흩어져 있던 관용구 정리) | 문서 없음(2026-08-29, 사용자 제보 배치 — `docs/webmanager.md`/`docs/webmanager-config.md`에 동작 변경분 반영, VNC 접속 클라이언트 목록/종료는 `.claude/backlog/vnc-connected-clients.md`로 분리) |
| 전체 화면 앱(대체 화면 버퍼)에서의 터치 스크롤 — 모바일에서 `claude` 같은 앱을 터치로 스크롤하면 앱에 스크롤이 전달되지 않고 xterm 뷰포트만 엉뚱하게 움직이던 문제. 터치 드래그를 `term.scrollLines()`로 직접 처리하던 것을 `.xterm-screen`에 합성 `WheelEvent`를 쏘는 방식으로 바꿔서, 마우스 보고/방향키 변환/뷰포트 스크롤 중 무엇을 할지는 xterm.js 자신의 wheel 처리(`CoreBrowserTerminal`)가 그대로 고르게 함 — 일반 버퍼에서는 손가락과 1:1로 움직이는 기존 경로 유지. 여기에 서버 쪽 짝(`internal/termsession/modes.go`, 신규): 스크롤백 링버퍼는 고정 크기 raw 바이트라 앱을 대체 화면으로 넘긴 `ESC[?1049h` 한 줄이 금방 밀려나고, 앱은 SIGWINCH에 다시 그리기만 할 뿐 모드를 다시 안 내보내므로 `nudgeRepaint`로도 복구가 안 됨 — 재접속한 클라이언트가 앱은 멀쩡히 그리면서 정작 버퍼는 다른 곳(그래서 매 리드로우가 스크롤백에 쌓임)에 있었음. pump()가 지나가는 바이트에서 DEC private mode set/reset을 증분 파싱해 두었다가 `Attach()`가 `Replay{Preamble, Scrollback}`으로 돌려주고, 리플레이 clear 앞에 붙여 보냄(대체 화면/괄호 붙여넣기/마우스 보고/커서 표시/DECCKM/키패드 등). 이어서 같은 링버퍼 문제를 겪는 나머지 상태도 전수 조사해서 추가 — vim 실기동 캡처(컨테이너에서 PTY로 뜬 실제 바이트)에서 드러난 DECSTBM 스크롤 영역(`ESC[1;24r`)과 키 수정자 리소스(`ESC[>4;2m`), xterm.js 소스 전수 조사에서 나온 ANSI 모드 4/20, DEC 6/9/45, DECSCUSR 커서 모양, 문자셋 지정 G0~G3 + SI/SO·locking shift(박스 그리기). 색(OSC 4/10/11/12)·창 제목·2026(동기 출력)·DECSC/제목 스택은 근거를 달아 일부러 제외 — 순수 외형이거나(그리고 `attach`가 쓰는 사용자의 진짜 터미널에는 되돌릴 기본값이 없음), 되살리면 오히려 화면이 얼어붙음. `attach` CLI는 detach 시 `termsession.ResetModesSequence()`로 사용자의 진짜 터미널을 원복 | 문서: `docs/webmanager.md` Terminal 절(2026-08-29, 사용자 제보 — 실서버 브라우저에서 e2e 확인: 256KiB 링버퍼가 가득 찬 상태에서 preamble이 `ESC[?1049h`로 시작하고 스크롤백에는 없음, 재접속 후 xterm `buffer.active.type === 'alternate'`, 합성 터치 드래그가 `ESC[A` ×10 전송, 일반 버퍼는 키 전송 0에 뷰포트만 이동. 2차 확장분도 동일하게 e2e 확인 — vim 시작 시퀀스 12개 전부 preamble에 복원되고 스크롤백엔 남아있지 않음, 재접속한 xterm이 alternate/appCursorKeys/appKeypad/bracketedPaste/mouse=drag/sendFocus, scrollTop~Bottom=0~23(48행 터미널에 앱이 걸어둔 1;24 그대로), cursorStyle=block, 마우스 보고가 켜진 앱에서는 터치 드래그가 방향키 대신 SGR 휠 보고 `ESC[<64;x;yM`로 나감) |
| 스크롤백 리플레이가 클립보드를 덮어쓰던 버그 — 탭 A에서 복사 → 탭 B에서 복사 → 다시 A로 돌아오면 A에서 복사했던 내용이 클립보드에 되돌아오던 문제(사용자 실측). 링버퍼는 raw 바이트라 OSC 52가 그대로 들어가 있고, 재접속 때 `writeScrollback`이 그걸 다시 흘려보내면 프론트의 OSC 52 핸들러가 한 번 더 실행됨 — 그리기가 아니라 **동작**인 시퀀스라 리플레이가 부작용을 낳는 유일한 케이스. `internal/termsession/oscfilter.go`(신규 `clipboardFilter`)를 pump에서 **링에 넣는 바이트에만** 물려서 OSC 52를 제거, 라이브 sink로 나가는 청크는 그대로라 복사 자체는 정상 동작. 리플레이 시점이 아니라 입력 시점에 거르는 이유는 링이 앞에서부터 밀려나 스냅샷이 시퀀스 중간에서 시작할 수 있기 때문(머리 없는 OSC 52 꼬리를 만나거나, 버려진 payload 꼬리를 시작으로 오인) — `modeTracker`와 같은 끊기지 않은 스트림을 보므로 PTY 읽기 단위로 쪼개진 시퀀스도 하나로 인식. OSC 0/7/8/152, DCS/APC는 그대로 통과. 범위는 일부러 OSC 52만 — `ESC[6n` 같은 질의 리플레이도 같은 부류지만 그건 터미널 안쪽 잡음이고, 이건 사용자가 소유한 바깥 상태를 덮어쓰는 유일한 것 | 문서: `docs/webmanager.md` Terminal 절(2026-09-08, 사용자 제보 — 사용자가 직접 빌드/배포해 실측 확인 완료) |
| code-server 내장 확장(`webmanager/vscode-extension/`) + 임베드 모드(`?embed=vscode`, `src/embed.ts`) — 터미널 세션을 에디터 탭(분할 가능)과 바닥 패널 슬롯(터미널 4 + 아무 탭 2, 패널이 바닥이면 나란히)으로, 다른 모든 탭도 "Open webmanager Tab…"으로 띄움. 새로고침/webmanager 재시작 후 같은 이름·마지막 cwd로 복원(끝난 세션은 "종료" 카드), 같은 세션은 창당 하나(재선택 시 기존 뷰로 이동), 뷰 액션으로 이름 변경/고정, 지정 단축키만 VS Code로 넘김(설정 가능), 새 창·`target=_top` 링크는 확장이 대신 엶, VS Code 테마 추종, 한 뷰에서 잠금 해제하면 다른 뷰/탭도 풀림(`BroadcastChannel`). 이미지 빌드 때 `.vsix`로 묶이고 code-server 시작마다 빌드 해시로 동기화(`config/code/code-extensions.default.sh`, `CODE_WEBMANAGER_EXTENSION=false`로 제거) | `.claude/qa-request/code-server-embed-plan-done.md` (실 HTTPS 환경에서 복사·지문 확인 대기) |
| 비밀번호 게이트 지문(WebAuthn) 잠금 해제 — 서버 측 WebAuthn(`internal/webauthnunlock`, go-webauthn), 비밀번호로 푼 직후 등록 제안/사이드바 지문 아이콘에서 기기 관리, 잠금 해제 창의 지문 버튼(클릭으로 열린 창이면 자동 1회 시도, 취소 시 비밀번호로 폴백 + 재시도), 호스트별 등록, 사용자 확인 필수, 비밀번호는 48시간마다 한 번 필요(2026-09-25 12시간에서 연장 — 세션 12시간 상한과 분리), 비밀번호 해시 변경 시 전부 해제, `WEBMANAGER_WEBAUTHN_ENABLED`로 끔 | `.claude/qa-request/webauthn-unlock-done.md` (가상 인증기로 e2e 확인, 실기기·passkeyd 확인 대기) |
