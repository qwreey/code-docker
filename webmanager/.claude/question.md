...
## 2026-09-06 배치(사용자 노트 8건) — 사용자 답변 완료

같은 날 실기기 테스트와 함께 전부 답변받음. 아래는 결과만 남긴 기록이고,
더 이상 열린 질문이 아니다.

| 항목 | 답 |
|---|---|
| 입력 방식 기본값 / 예전 opt-out 존중 | **존중하지 않음** — 옛 boolean 키 마이그레이션을 아예 제거하고 전 기기 `diff` 기본 |
| 터치 모드 선택기 위치(상단 헤더) | 그대로 OK |
| 터치 모드 기본값 `scroll` | OK |
| 터미널 탭에서만 앱 상단 바 숨김 | OK |
| 파일 에디터 줄바꿈 기본 켬 | OK |
| VNC kick 창 15초 | OK |
| VNC 네이티브 클라이언트 가시성 | **필요 없음** — router를 경유하는 연결만 관리 대상. `wayvncctl` 연동은 하지 않음 |
| 핀치 줌 | 지금은 하지 않음, 백로그 유지(`terminal-pinch-zoom-plan.md`) |
| 탭바 가로 스크롤 | **원함**, 특히 모바일. 구현함(아래) |
| authgate per-IP 백오프 | 지금 안 고침, 레포 루트 `TODO.md`로 이동 |

## (구) 2026-09-06 배치에서 남은 결정 — 위 표로 대체됨
