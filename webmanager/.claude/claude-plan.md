...
아래 "로그인(OAuth) 연계 — 조사 결과" 절에서 최후순위로 미뤄뒀던 로그인 자동화를
이번 라운드에 실제로 구현함. **결정을 뒤집은 이유**: webmanager가 이제 "code-server나
SSH는 전혀 열지 않고 webmanager만 노출하는" 배포 시나리오(사내/조직 배포)까지
지원해야 해서, "사용자가 터미널에서 직접 `claude auth login`을 실행한다"는 원래
전제가 더 이상 항상 성립하지 않게 됨 — 로그인할 터미널 자체가 없을 수 있음.

미해결로 남겨뒀던 "파이프만으로 충분한지, pty가 필요한지" 질문은 직접
실험으로 해소: `claude auth login`을 stdin `/dev/null`(non-tty), 임시
`$HOME`/`CLAUDE_CONFIG_DIR`로 실행해 URL 출력까지 확인 후 `timeout`으로 종료(코드
붙여넣기 전 종료 — 실제 인증 완료 없음, 크리덴셜 미접촉). **결론: 일반
`os/exec` 파이프로 충분, pty 불필요** — Terminal 기능이 이미 `creack/pty`를
의존성으로 갖고 있지만 이번 기능엔 일부러 안 씀.

백엔드: `internal/claudecode/login.go`의 `LoginManager`(단일 세션 슬롯 — 새
`Start`가 이전 세션을 kill하고 대체, `mise.JobStore`와 달리 TTL 기반 정리 불필요)가
`claude auth login`을 `context.Background()` + 자체 10분 타임아웃으로 백그라운드
실행(HTTP 요청 컨텍스트에 안 묶음 — 사용자가 브라우저 갔다 오는 동안 요청은 이미
끝나 있음), `StdinPipe`/`StdoutPipe`/`StderrPipe`를 각각 goroutine으로 스캔하며
`urlRe`(`https://\S+`, 문구 프리픽스 대신 URL 패턴만 매칭 — 위 미해결 질문 3의 답:
CLI 버전 업데이트로 문구가 바뀌어도 안 깨지도록 일부러 느슨하게)로 첫 URL을 캡처.
`handlers_claude.go`에 4개 핸들러(전부 `gate.RequirePassword`로 게이팅 — 살아있는
인증 플로우라 Terminal과 동급 신뢰 등급), `main.go`에 라우트 등록. 프론트는 별도
에이전트가 이 계약대로 병행 작업 중.

`gofmt`/`go build`/`go vet` 전부 클린 확인.
...
- **M5 (기술적으로 까다로워서 뒤로 미룸)**: MCP 서버 목록. `claude mcp list`가
  ...
  출력을 관찰한 뒤에 설계할 것.
- ~~**로그인(OAuth) 자동화**~~: 완료(위 "구현 완료 (2026-08-03)" 참고) — 원래는 이
  마일스톤 목록에서 빼고 사용자가 직접 터미널에서 진행하기로 했었으나, webmanager가
  code-server/SSH를 전혀 안 여는 배포 시나리오까지 지원해야 해서 결정을 뒤집음.

**M1 기준 다른 미해결 사항 없음** — "미해결 질문" 절에 남아있던 항목은 전부 M5(MCP
파싱) 관련. 로그인 관련 미해결 질문은 구현하며 전부 해소(아래 "로그인(OAuth) 연계"
절 참고).
...
- **확장 설치**: `webmanager/ideas.md`에 후순위로 보류돼 있던 "익스텐션 목록
  ...
  저장소" 참고).
- ~~**로그인(OAuth) 자동화 — 최후순위, 이번 스코프 아님**~~: 완료(2026-08-03, 위 참고).
  브라우저 콜백을 SSH 포트포워딩으로 우회할 필요 자체가 없다는 걸 확인함 — `claude`
  CLI가 이미 "콜백을 못 받으면 코드를 화면에 보여주고 터미널에 붙여넣게" 하는 폴백을
  컨테이너/SSH 환경을 염두에 두고 내장하고 있음. 원래는 이 조사 결과만 남겨두고
  webmanager에 로그인 실행 UI를 만드는 건 스코프에서 뺐었으나(사용자가 직접 터미널에서
  진행하는 쪽으로 확정했었음), code-server/SSH를 전혀 안 여는 배포 시나리오까지
  지원해야 한다는 요구가 생겨 뒤집음 — 아래 "로그인(OAuth) 연계" 절 참고.
...
## 로그인(OAuth) 연계 — 조사 결과 및 구현 (2026-08-03 구현 완료)

> 이 절은 원래 **최후순위로 미룬** 로그인 자동화를 나중에 사용자가 직접 진행할 때
> 참고할 조사 기록으로만 남겨뒀던 것인데, webmanager가 code-server/SSH를 전혀 열지
> 않는 배포(사내/조직 시나리오)까지 지원해야 하게 되면서 이번 라운드에 실제로
> 구현함. 아래 조사 내용은 여전히 유효하고, 구현은 조사 결과를 그대로 따름 — 실제
> 코드는 `internal/claudecode/login.go` + `handlers_claude.go`의 `handleClaudeLogin*`
> 4개 핸들러, 라우트는 `POST /api/claude/login/start`, `GET /api/claude/login/{id}`,
> `POST /api/claude/login/{id}/code`, `POST /api/claude/login/{id}/cancel` (전부
> `gate.RequirePassword`로 게이팅).
...
## 로그인 관련 질문 — 해소 상태 (구현 완료, 2026-08-03)

2. ~~`claude auth login` 서브프로세스를 파이프로만 다뤄도 URL/코드 프롬프트가 정상
   노출되는지, 아니면 pty가 꼭 필요한지.~~ **해소**: 직접 실험으로 확인 —
   `claude auth login`을 stdin `/dev/null`(non-tty)로 실행해도 URL이 stdout에 정상
   출력됨. 일반 `os/exec` 파이프(`StdinPipe`/`StdoutPipe`/`StderrPipe`)로 충분, pty
   불필요. 실측된 마지막 줄(`Paste code here if prompted >`)은 개행이 없는 프롬프트라
   `bufio.Scanner`(`ScanLines`)가 프로세스 생존 중엔 `lines`에 못 담을 수 있음 —
   의도적으로 허용: 프론트는 이 문자열이 아니라 추출된 URL로 준비 상태를 판단.
3. ~~로그인 URL을 stdout에서 정규식으로 뽑아내는 게 버전에 안전한지~~ **해소**:
   `urlRe = regexp.MustCompile(`https://\S+`)`로 안전 마진을 둠 — "If the browser
   didn't open, visit:" 같은 문구 프리픽스는 매칭하지 않고 URL 패턴만 봐서, 문구가
   바뀌어도 파싱이 안 깨짐.
4. ~~로그인 중 사용자가 중간에 webmanager 탭을 닫거나 새로고침하면 대기 중이던
   서브프로세스를 어떻게 정리할지~~ **해소**: `LoginManager`가 세션 시작 시
   `context.WithTimeout(context.Background(), 10*time.Minute)`로 최대 수명을 강제(방치돼도
   영구히 안 남음), 프론트는 언마운트 시 `POST /api/claude/login/{id}/cancel`을
   best-effort로 호출(이미 끝난 세션이어도 항상 200, 멱등). 세션은 하나만 유지되고
   새 `Start`가 이전 세션을 kill하고 대체.
