...
## 나중 마일스톤 (질문 1·2는 해결됨, 질문 3·4는 아직 미착수)
...
**구현 완료 (2026-08-04)**: `config/code-patch/webmanager-launcher.default.js`
(신규) — 아래 질문 1·2에 대한 답을 실제로 구현함.

1. ~~**트리거**: 커맨드 팔레트 항목? 사이드바에 상시 아이콘? 키바인딩?~~ —
   **타이틀바 좌상단 `.window-appicon` 클릭**으로 결정. hover 시 밝아지는
   효과 + `title="웹매니저 열기"` 툴팁을 붙여서 클릭 가능한 요소임을
   드러냄. `code-server-autoinstall`의 `write_window_appicon()`이 이미
   커스터마이징하는 실존 DOM 클래스를 그대로 재사용(서브모듈은 안 건드림).
2. ~~**표시 형태**: 작은 위젯 패널(iframe) vs 전체 화면 전환~~ — **위젯
   쪽으로 결정**: `position:fixed` dim 배경 위에 90vw×90vh 둥근 모달로
   `<iframe src="/manager/">`를 띄움(전체 화면 네비게이션이 아니라 code-server
   화면 위에 겹쳐 뜨는 오버레이). 우상단 ✕ 버튼/ESC/배경 클릭으로 닫힘,
   아이콘 재클릭으로 토글. `CDDialog`(토스트/배너 전용 모듈)는 재사용하지
   않고 독립 구현 — 스타일 주입 방식(`<style>` 태그 1회 생성)만 그 패턴을
   따름.
