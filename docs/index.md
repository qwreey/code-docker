...
## 터미널 세션에 밖에서 붙기 (attach)

webmanager의 [Terminal](webmanager.md#terminal) 탭에서 돌고 있는 세션에, 브라우저가 아닌 곳(ssh, code-server 통합 터미널 등)에서 그대로 붙는 명령입니다. tmux 같은 별도 장치가 아니라 브라우저 탭과 **똑같은 세션에 붙는 또 하나의 클라이언트**라서, 브라우저 탭과 동시에 붙어 있어도 되고 양쪽 입력이 그대로 공유됩니다.

```sh
attach <세션 이름> [시작 디렉터리]
attach '세션 1'          # 이름에 공백이 있으면 따옴표 필수
```
...
> `Ctrl+D`는 빠져나오는 키가 아닙니다. 셸을 종료시키는 것이라 **세션 자체가 끝나고, 그 세션을 보고 있던 브라우저 탭도 같이 사라집니다.**
...
- **빌드**: `build.*.sh`
- **code-server**: `code-service.*.sh`, `code-config.*.yaml`, `code-env.*.sh`, `code-runner.*.sh`, `settings.*.json`(사용자 `settings.json` 초기값 — 없을 때만 시드, 덮어쓰지 않음), `recommendations.*.yaml`
