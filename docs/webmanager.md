...
[router.md](https://github.com/qwreey/router-docker/blob/HEAD/docs/router.md#보안-공유-origin과-전용-도메인router_manager_hosts) 참고). 이름(내부
...
[dev 서버 노출 문서](https://github.com/qwreey/router-docker/blob/HEAD/docs/dev-proxy.md)를 확인하세요.
...
바꾸면 다시 생성되지 않습니다. 자세한 내용은 [app-routes.md](https://github.com/qwreey/router-docker/blob/HEAD/docs/app-routes.md)를
확인하세요.
...
때문입니다). 탭은 터미널처럼 화면을 꽉 채우고, 목록에서 "열기"를 누르면
대상이 탭으로 열립니다 — 여러 대상을 동시에 열어 두고 오갈 수 있고, 탭을
닫으면 연결이 끊깁니다. **webmanager의 다른 메뉴로 갔다 와도 연결은 유지**되고
(VNC 탭을 한 번 열면 숨겨 둔 채 살려 둡니다), 보고 있는 대상은 주소의
`?target=<이름>`에 남아서 새로고침·북마크하면 그 대상이 다시 열립니다.
네이티브 VNC 클라이언트로 붙고 싶다면 대신 아래 Net 관리 탭의 Forwards를
쓰세요. 대상마다 지금 붙어 있는 클라이언트 수가 배지로 뜨고, 펼치면 IP/브라우저/접속
시각과 함께 개별 "끊기" 버튼이 있습니다 — 어느 기기에서 켜놨는지 몰라 화면 크기가
서로 밀리는 상황을 정리하기 위한 것입니다. **router를 거친 연결만 보입니다**(대상의
RFB 포트에 직접 붙은 네이티브 클라이언트는 여기 안 잡힙니다). 자세한 내용은 [vnc.md](https://github.com/qwreey/router-docker/blob/HEAD/docs/vnc.md)를 확인하세요.
...
호출할 뿐입니다. 자세한 내용은 [router.md](https://github.com/qwreey/router-docker/blob/HEAD/docs/router.md#tailscale)를 확인하세요.
...
포함되어 있습니다. 자세한 내용은 [router.md](https://github.com/qwreey/router-docker/blob/HEAD/docs/router.md)와
[egress-netgate.md](https://github.com/qwreey/router-docker/blob/HEAD/docs/egress-netgate.md)를 확인하세요.
...
자세한 내용은 [egress-netgate.md](https://github.com/qwreey/router-docker/blob/HEAD/docs/egress-netgate.md)를 확인하세요.
...
[router.md](https://github.com/qwreey/router-docker/blob/HEAD/docs/router.md#tinyauth)를 확인하세요.
...
[router.md](https://github.com/qwreey/router-docker/blob/HEAD/docs/router.md#router-manager-자체-인증)를 확인하세요.
...
로그인 흐름. 아직 이 컨테이너에서 `claude`를 한 번도 온보딩한 적이 없다면 실제
`claude` CLI를 그대로 터미널로 띄워서(xterm.js) 테마 선택부터 로그인까지 화면
안내를 따라가는 방식이 기본으로 뜹니다 — 헤드리스로 `claude auth login`만 돌리면
로그인 자체는 되지만 code-server 통합 터미널에서 맨 `claude`를 쳤을 때 온보딩이
끝났다는 표시가 안 남아 첫 실행 화면이 계속 다시 뜨는 문제가 있었기 때문입니다.
온보딩을 마친 상태에서는 이 터미널 없이도 링크만 열고 코드만 붙여넣는 간단한
브라우저 로그인이 기본으로 뜨고, 필요하면 "고급" 옵션으로 터미널 로그인도 계속 쓸
수 있습니다. 온보딩은 한 번으로 끝나지 않습니다. `claude` 안에서 `/logout`을
실행하면 온보딩 완료 표시도 함께 지워지고(패널의 로그아웃 버튼인
`claude auth logout`은 지우지 않음), 로그인 도중 창이 끊겨 마지막 화면까지 못 가도
그대로 남습니다. 이렇게 "로그인은 됐는데 온보딩은 안 끝난" 상태면 로그인 상태
카드에 경고와 "온보딩으로 다시 로그인" 버튼이 뜹니다. 브라우저 로그인만 다시
해서는 복구되지 않고, 터미널 온보딩을 마지막 화면까지 끝내야 합니다. 터미널 로그인 중 코드를 복사하러 탭을 옮겨
연결이 끊겨도 서버 쪽 로그인은 계속 살아 있어서, 돌아오면 같은 화면에 다시
붙습니다(시작부터 최대 10분). mise로 관리 중인 버전이 최신인지 확인해서
...
바로 해당 세션으로 이동 가능 — 개요 바로 아래, 기본 펼침)이 함께 표시됩니다. 세션
기록의 각 항목에는 "터미널에서 이어하기" 버튼이 있어, 그 대화의 작업 디렉터리에서
...
실시간 cwd가 홈 화면/탭 툴팁에 표시되고, 거기서 바로 해당 프로젝트 정보를 다이얼로그로
띄울 수 있습니다 — 터미널을 보던 화면을 벗어나지 않게 탭 이동이 아니라 그 자리에 뜨며,
Projects 탭 상세 시트와 같은 패널들(열린 세션/Git/워크트리/세션 기록/메모리)을 그대로
재사용합니다. 다시 스캔·삭제 같은 관리 동작은 빠져 있고, 필요하면 헤더의
"Projects 탭에서 열기"로 넘어갑니다(Projects 탭 상세 시트에도 반대로 그 프로젝트 아래
열린 세션 목록이 뜹니다).
모바일에서도 쓸 수 있게 Esc/Ctrl/Alt/Shift/Tab/방향키 온스크린 버튼(커스터마이징
가능)과 색상 테마(프리셋 10개 + 사용자 정의), 폰트 선택(Fonts 탭) 지원.
`claude`/`vim`/`htop`처럼 화면 전체를 쓰는 앱(대체 화면 버퍼) 안에서는 터치 스크롤이
터미널 화면을 움직이는 대신 앱에 전달할 스크롤 입력(앱이 마우스를 켜뒀으면 휠 보고,
아니면 위/아래 키)으로 바뀝니다 — 데스크탑에서 휠을 굴렸을 때와 같은 동작이고, 일반
셸 화면에서는 그대로 스크롤됩니다. 설정 패널의 "모바일 입력 · 스크롤 (실험적)"에서 끌
수 있습니다.

한 손가락 드래그가 무엇을 할지는 터미널 상단의 **터치 모드**(스크롤 / 마우스 / 선택)로
고릅니다(터치 기기에서만 보입니다). `마우스`는 드래그를 진짜 마우스 입력으로 앱에
전달하고(tmux 창 선택, vim/htop 마우스 모드, claude의 클릭형 프롬프트), `선택`은
드래그로 텍스트를 선택합니다 — 선택이 생기면 상단에 "복사" 버튼이 뜹니다. 모바일에는
Ctrl+C도 쓸 만한 네이티브 선택도 없어서, 이게 터미널 내용을 복사하는 경로입니다.
두 모드 모두 **터치한 지점부터** 선택·드래그가 시작되고, 움직이지 않고 **꾹 누르고 있으면**
짧은 진동과 함께 그 자리에서 드래그가 시작됩니다. 드래그나 스크롤을 하는 동안에는 키보드가
열리지 않고, 끝나고 잠시 뒤에 탭하면 평소처럼 열립니다.

`스크롤` 모드에서 빠르게 튕기면 손을 뗀 뒤에도 **관성으로 계속 스크롤**되다가 서서히
멈춥니다(다시 터치하면 그 자리에서 멈춤). vim(`mouse=a`)·tmux처럼 마우스를 켠 전체 화면
앱에서도 관성이 휠 스크롤로 이어지고, 마우스를 켜지 않은 전체 화면 앱에는 일부러 붙지
않습니다 — 그쪽은 스크롤이 방향키 입력이라 손을 뗀 뒤에도 방향키가 계속 들어가기
때문입니다. 설정 패널에서 끌 수 있습니다.

모바일 가상 키보드는 예측 입력 때문에 터미널과 잘 맞지 않고, 기기/키보드 앱마다
증상이 달라서 설정 패널에서 **입력 방식 세 가지** 중에 고를 수 있습니다.
기본값인 **새 입력 필드**는 입력을 숨겨진 필드에 쌓아두고 이미 보낸 내용과의 차이만
전송해서, 키보드가 이미 확정한 문장을 다시 흘려보내도 중복 입력이 생기지 않게 합니다.
한글은 **치는 동안 조합 과정이 그대로 보이고**(`ㅇ`→`아`→`안`), 조합 중 백스페이스는
자모 단위, 완성된 글자는 글자 단위로 지워집니다. 같은 패널의 "띄어쓰기마다 입력 필드를 다시
포커스"(기본 켬)는 단어가 끝날 때 키보드의 자동완성 연관을 끊습니다 — 기기에 따라 띄어쓰기마다
키보드가 깜빡이면 끄세요. **비밀번호 필드**는 예측 입력을 확실히 끄는 대신 한글이
자모로 쪼개지고, **끔**은 xterm 기본 동작입니다. 그래도 이상하면 같은 패널의
"입력 디버그 표시"를 켜세요. 입력 이벤트와 실제로 보낸 바이트, 화면 크기 변경, 관성
스크롤 판단이 터미널 위에 찍히고, 오버레이의 **복사** 버튼으로 기기·브라우저 정보와 함께
텍스트로 가져갈 수 있습니다(평문 http라 클립보드 복사가 막히면 전체가 선택된 칸이 대신 뜹니다).

태블릿에 물리 키보드(키보드 커버 등)를 붙여 쓸 때도 방향키·Home/End·Tab·Esc·F키·Ctrl/Alt 조합이
터미널로 그대로 들어갑니다(Ctrl+V는 붙여넣기로 남겨둡니다).

하단 컨트롤 바의 버튼은 **이미 열려 있는 키보드를 유지만** 하고 닫힌 키보드를 열지는
않습니다 — 확대/축소만 하려는데 키보드가 올라오는 일이 없습니다. 키보드를 열고 닫아 터미널
크기가 바뀌면 vim·claude 같은 전체 화면 앱은 크기가 자리 잡은 뒤 한 번 더 다시 그려집니다.

모바일에서는 터미널 탭에 한해 앱 상단 바가 사라지고 그 햄버거 메뉴가 "Terminal" 왼쪽에
붙습니다. 키보드가 올라온 상태에서 세로 공간을 한 줄만큼 더 벌기 위한 것입니다.

지금 보고 있는 세션은 주소(`?session=...`)에 담기므로, 새로고침해도 같은 세션으로
돌아오고 북마크도 됩니다. 그 세션이 이미 사라졌으면 새로 만들지 않고 홈 탭으로
돌아갑니다.
세션에 다시 붙을 때는 스크롤백을 되감기 전에, 그 세션이 지금 걸어둔 터미널 상태를 먼저
복원합니다 — 대체 화면 버퍼, 괄호 붙여넣기, 마우스 보고, 커서 표시/모양, 커서키·키패드
모드, 스크롤 영역(DECSTBM), 문자셋 지정(박스 그리기), 키 수정자 리소스 등. 스크롤백
버퍼는 고정 크기라(`WEBMANAGER_TERMINAL_SESSION_SCROLLBACK_BYTES`) 앱이 뜰 때 딱
한 번 내보내는 이 설정들은 금방 밀려나 버리고, 앱은 화면을 다시 그릴 뿐 설정을 다시
내보내지 않기 때문에 그것만 보고 있으면 앱은 멀쩡히 그려지는데 브라우저 쪽 상태만 다른
곳에 있게 됩니다. 색(팔레트/전경·배경)과 창 제목은 일부러 복원하지 않습니다 — 고장이
아니라 보기의 문제이고, `attach`가 쓰는 사용자의 진짜 터미널에서는 detach할 때 원래
값으로 되돌릴 방법이 없기 때문입니다.
반대로 클립보드 복사(OSC 52)는 스크롤백에 **남기지 않습니다**. 이것만은 화면을 그리는
시퀀스가 아니라 동작이라, 그대로 두면 다시 붙을 때마다 그 복사가 한 번 더 실행되면서
지금 클립보드에 있는 내용을 예전 것으로 덮어씁니다 — 탭 A에서 복사하고, 탭 B에서 복사한
뒤, 다시 A로 돌아오면 A에서 복사했던 게 되돌아오는 식입니다. 실시간 출력은 그대로이므로
복사 자체는 평소대로 동작하고, 화면에 보이는 것도 달라지지 않습니다.
여기서 돌고 있는 세션에는 브라우저 밖(ssh, code-server 통합 터미널)에서도
`attach <이름>`으로 붙을 수 있습니다 —
[터미널 세션에 밖에서 붙기](index.md#터미널-세션에-밖에서-붙기-attach) 참고

#### code-server 안에서 쓰기 (내장 확장)

code-server에는 webmanager 확장이 기본으로 깔려 있어서, webmanager 터미널(과 다른
모든 탭)을 code-server 창 안에 바로 띄울 수 있습니다. code-server 자체 통합 터미널에서
`attach`하는 것과 달리 복사·스크롤·테마가 webmanager 터미널 그대로입니다.

- **에디터 탭**:
  - 커맨드 팔레트 `webmanager: Open Terminal Session…` / `…to the Side…`로 엽니다.
  - 에디터를 분할해 터미널 두 개를 나란히 놓을 수 있습니다.
  - 새 세션은 마지막으로 본 에디터 파일이 속한 워크스페이스 폴더에서 시작합니다. 폴더가 여럿인 워크스페이스에서는 폴더를 고르게 하고, 그 폴더를 맨 위에 "현재 에디터의 폴더"로 표시합니다. 바닥 패널의 터미널 홈에서 "새 세션"을 눌러도 똑같이 동작합니다.
  - `webmanager: Open webmanager Tab…`으로는 Files, VNC, Logs 같은 다른 탭도 에디터 탭으로 열립니다.
  - VNC는 터미널처럼 **대상마다 에디터 탭 하나**입니다. `webmanager: Open VNC…`(바닥 패널은 `…in Panel…`)에서 대상을 고르면 그 대상의 화면만 담은 탭이 뜨고, 탭 제목은 대상에 설정된 이름입니다. 이미 열린 대상을 다시 고르면 그 탭으로 이동할 뿐 연결을 하나 더 만들지 않습니다. 같은 목록의 **대상 목록**은 추가·편집·접속자 관리용 뷰이고, 거기서 "열기"를 눌러도 대상이 각자의 탭으로 열립니다. "새 창"은 그 대상의 에디터 탭을 닫아 연결을 넘긴 뒤 브라우저 새 창으로 엽니다(웹뷰 안에서는 팝업이 막혀 있어서 확장이 대신 엽니다). 확장은 router의 API에 직접 닿지 못해서 대상 목록을 뷰가 알려 준 것으로 기억합니다 — 처음 한 번은 목록이 비어 있을 수 있고, 그때는 **대상 목록**을 열면 채워집니다.
- **바닥 패널**:
  - `…in Panel…` 명령을 쓰면 바닥 패널의 **WEBMANAGER** 탭에 뜹니다. 터미널은 네 개까지, 다른 탭은 두 개까지입니다.
  - 패널이 바닥에 있으면 나란히 놓이고(예: `antigravity`와 `claude`를 한 프로젝트에서 동시에), 각 뷰는 사이드바로 끌어다 옮길 수 있습니다.
  - 아무 세션도 연결되지 않은 첫 칸에는 터미널 홈이 떠서, 저장해 둔 프로파일을 바로 띄울 수 있습니다.
- **복원**:
  - 브라우저를 새로고침해도 같은 세션으로 다시 붙습니다.
  - 그사이 세션이 사라졌으면(30분 유휴 정리, webmanager 재시작) 같은 이름으로 마지막 위치에서 새로 엽니다.
  - `exit`로 셸을 끝낸 경우에만 "세션이 종료되었습니다" 카드가 뜨고, 거기서 다시 시작하거나 닫을 수 있습니다.
- **같은 세션은 창 하나에 한 번만 열립니다.** 이미 열린 세션을 다시 고르면 그 뷰로 이동합니다.
- **URL·커밋 열기**(브라우저와 code-server 뷰 모두): 터미널 출력의 URL을 Ctrl(맥은 Cmd)+클릭하면 새 탭으로 열리고, 커밋 해시(`git log --oneline`의 `2ab4c9b` 등)를 Ctrl+클릭하면 세션이 있는 프로젝트의 정보 창이 그 커밋의 변경 내용이 펼쳐진 채로 열립니다. 해시 링크는 세션이 알려진 프로젝트 폴더 안에 있을 때만 생깁니다.
- **파일 열기**: 터미널 출력의 파일 경로를 Ctrl(맥은 Cmd)+클릭하면 code-server 에디터에서 열립니다. `src/foo.ts:12:5`, `foo.py(12,5)`처럼 줄·칸이 붙어 있으면 그 위치로 가고, 상대 경로는 세션의 현재 디렉터리 기준입니다. 해당 파일이 없으면 그 글자를 넣은 Quick Open(Ctrl+P)이 열리고, 디렉터리는 탐색기에서 보여 줍니다.
- **테마·지문·잠그기**: 뷰는 기본으로 VS Code 테마(라이트/다크)를 따라갑니다. 고정하려면 명령 팔레트의 `webmanager: Theme…`(설정 `webmanager.theme`)를 씁니다. 사이드바에 있던 지문 기기 관리와 잠그기는 뷰의 `…` 메뉴와 명령 팔레트(`webmanager: Fingerprint Unlock Devices…`, `webmanager: Lock Now`)에 있습니다.
- **다른 탭으로 전환**: 에디터 탭과 바닥 패널의 탭 뷰(터미널 외)는 제목 줄의 목록 아이콘(`Switch webmanager Tab…`)으로 같은 자리에서 다른 webmanager 탭으로 바꿉니다. 에디터 탭은 터미널 세션으로도 바꿀 수 있고, 터미널 뷰는 `Change Session…`으로 세션을 바꿉니다.
- **이름 변경·고정·브라우저에서 열기·다시 불러오기**: 패널 뷰는 제목 줄 `…` 메뉴, 에디터 탭은 제목 줄 메뉴와 커맨드 팔레트에서 합니다. 고정과 글자 확대/축소(하단 컨트롤 바가 꺼져 있을 때)는 터미널 상단 바에도 버튼이 있습니다. 터미널을 브라우저에서 열면 뷰는 닫힙니다(한 세션을 두 곳에서 보면 크기가 서로 싸우기 때문).
- **단축키**:
...
주의:
- code-server를 **HTTPS(또는 localhost)**로 열어야 합니다. 평문 http의 LAN 주소에서는 VS Code 웹뷰 자체가 뜨지 않습니다(확장이 경고 알림을 띄웁니다).
- 숨겨진 터미널·VNC 뷰도 연결을 유지합니다. 뷰마다 webmanager 한 벌이 떠 있는 셈이라 많이 열수록 브라우저 메모리를 씁니다. 설정 `webmanager.retainContextWhenHidden`으로 바꿀 수 있습니다.

확장은 이미지 빌드 때 함께 만들어지고, code-server가 시작될 때마다 이미지에 든 빌드로
맞춰집니다(설치·업데이트·다운그레이드, Extensions 화면에서 지워도 다음 시작 때 다시
깔림). 끄려면 `.env`에 `CODE_WEBMANAGER_EXTENSION=false`를 두세요 — 지워지고 다시
깔리지 않습니다.
...
편집기는 전체 화면으로 열리고, 상단에 **줄바꿈 토글**이 있습니다(기기별로 저장됨) —
md처럼 긴 줄을 가로 스크롤 없이 읽기 위한 것입니다.

지금 보고 있는 폴더는 주소(`?path=...`)에 담기므로 새로고침/북마크가 됩니다.
Projects 탭의 프로젝트 상세도 마찬가지입니다(`?project=...`).

터미널 탭의 "파일 브라우저에서 열기"는 탭을 옮기지 않고 **현재 화면 위에 전체 화면
오버레이**로 뜹니다 — 에이전트가 써둔 파일을 확인하러 갔다가 터미널로 돌아오는 왕복이
없어집니다. 계속 볼 거면 오버레이 안의 "탭으로 열기"로 넘어가면 됩니다.
