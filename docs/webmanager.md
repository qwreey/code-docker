...
Ctrl+C도 네이티브 선택도 없어서, 이게 터미널 내용을 복사하는 유일한 경로입니다.
...
(한글 조합도 정상적으로 됩니다). **비밀번호 필드**는 예측 입력을 확실히 끄는 대신 한글이
자모로 쪼개지고, **끔**은 xterm 기본 동작입니다. 그래도 이상하면 같은 패널의
"입력 디버그 표시"를 켜서 어떤 이벤트가 오고 실제로 어떤 바이트가 나가는지 화면에서
바로 볼 수 있습니다.

하단 컨트롤 바의 버튼은 **이미 열려 있는 키보드를 유지만** 하고 닫힌 키보드를 열지는
않습니다 — 확대/축소만 하려는데 키보드가 올라오는 일이 없습니다.
