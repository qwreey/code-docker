...
같은 서브도메인 안의 라우트는 등록한 순서대로 평가되므로, 좁은 path(`/api/*`)를 넓은 path(전체 매치)보다 먼저 두어야 합니다. 저장하면 `/code/.caddy-adapter/managed/<이름>.caddy` 파일 하나가 갱신되고, `caddy adapt`로 문법 검증 후 `caddy reload`로 무중단 반영됩니다 (검증 실패 시 반영 자체가 안 되고 에러가 그대로 표시됩니다).
