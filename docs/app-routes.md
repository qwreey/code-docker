...
Host 헤더와 무관하게 `/app/<이름>/...` 경로로 들어오는 요청을 지정한 대상으로
리버스 프록시하는 기능입니다(`/app/<이름>` 접두사는 자동으로 제거되고 나머지
경로만 대상으로 전달됩니다). [Dev Proxy](dev-proxy.md)가 도메인(Host 헤더)으로
분배하는 것과 반대로, App Routes는 **경로**만으로 분배합니다 — router는
자신보다 신뢰 수준이 낮은 code-docker의 실제 내부 앱 구조를 미리 알 필요가
없는 범용 국경 컨테이너([router.md](router.md) 참고)이고, 바깥에 붙는
리버스 프록시도 도메인마다 다른 Host 매칭 규칙을 새로 만들지 않고 그냥
경로 접두사만 붙여서 넘기면 되도록 만들어졌습니다.
...
- **target** (`host:port`) — 리버스 프록시 대상. router 컨테이너 기준으로
  ...
  풀 수 있지만, router 자기 자신은 이 옵트아웃으로도 절대 허용되지
  않습니다. Dev Proxy의 `DEVPROXY_ALLOW_EXTERNAL_TARGETS`와는 완전히
...
통해서만 접근 가능하도록 의도적으로 고정했습니다.

> 참고: 이 방식으로는 `code-server`를 `localhost`에서 직접 접근하기가
> 불편해질 수 있습니다(항상 `/app/code/` 접두사가 필요) — 이 기능이
> 커버하려는 범위 밖의 트레이드오프로 인지하고 있습니다.
