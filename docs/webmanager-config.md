...
게이트가 켜져 있으면 비밀번호 대신 **지문(또는 Face ID, 기기 PIN 등 기기의 사용자 확인)**으로도
풀 수 있습니다. 휴대폰·태블릿·노트북 어디서나 되고, Linux에서는 passkeyd(fprintd 지문)처럼
WebAuthn 인증기로 보이는 것이면 됩니다.

- **등록**: 비밀번호로 잠금을 푼 직후 "지문으로 잠금 해제할까요?" 창이 뜹니다. 방금 입력한 비밀번호로 바로 등록되므로 다시 묻지 않습니다. 나중에 하려면 사이드바 하단의 지문 아이콘을 누릅니다. 거기서 등록된 기기 목록을 보고, 지우고, 이 기기를 등록할 수 있습니다(이때는 비밀번호를 한 번 더 확인합니다).
- **잠금 해제**: 등록한 기기에서는 잠금 해제 창에 "지문으로 잠금 해제" 버튼이 생깁니다. 버튼을 눌러 연 창이면 바로 지문을 요청합니다. 취소하거나 실패하면 비밀번호 입력란이 그대로 있고, 버튼이 "지문으로 다시 시도"로 바뀝니다(자동으로 반복하지는 않음).
- **패스키로 저장됩니다.** 등록하면 기기의 패스키 제공자(Google 비밀번호 관리자, 삼성 패스 등)에 이 주소의 패스키가 생깁니다. 제공자 앱에서 패스키를 직접 추가할 수는 없고 이 등록으로만 만들어집니다. 등록하지 않은 기기에서 지문 버튼을 누르면 "다른 기기의 패스키(QR)" 안내만 나오는데, 이때는 비밀번호로 푼 뒤 등록하면 됩니다.
- **확인·잠그기**: 기기 목록의 "지문으로 시험해 보기"는 잠그지 않고 실제 지문 확인을 한 번 거쳐, 어느 등록 항목으로 인증됐는지 "이 기기"로 표시합니다. "지금 잠그기"(또는 사이드바의 잠금 해제 표시를 누르기)는 이 브라우저의 잠금 해제를 바로 끝냅니다. code-server 뷰에서는 사이드바가 없으니 뷰의 `…` 메뉴나 명령 팔레트의 `webmanager: Fingerprint Unlock Devices…` / `webmanager: Lock Now`를 씁니다.
- **비밀번호는 48시간마다 한 번은 필요합니다.** 지문으로 풀 때마다 새 세션이 시작되므로(위의 12시간 상한도 지문으로 푼 시점부터 다시 셉니다), 하룻밤 자고 와도 지문으로 풀립니다. 다만 마지막으로 비밀번호를 입력한 시점부터 48시간이 지나면 지문은 거절되고 비밀번호가 다시 필요하며, 그 전에 연 세션도 그 시점에 잠깁니다. 지문 잠금 해제는 비밀번호의 대체가 아니라 그 사이의 편의입니다.
- **비밀번호(해시)를 바꾸면 등록된 기기는 모두 해제됩니다.**
- **기기는 등록한 주소에서만 쓸 수 있습니다.** 공개 주소와 tailnet 주소처럼 호스트 이름이 다르면 각각 등록해야 합니다. 상위 도메인을 공유하는 다른 사이트(router vhost로 띄운 사이드 프로젝트 등)가 이 등록을 쓸 수 없게 하려는 것입니다.
- **HTTPS(또는 localhost) 주소에서만 동작합니다.** 평문 http나 IP 주소로 열면 버튼 자체가 나오지 않고 비밀번호만 씁니다.
- 서버에는 공개키만 저장됩니다(`WEBMANAGER_WEBAUTHN_PATH`, 기본 `/code/.local/share/code-docker/webmanager/webauthn.json`). 이 파일은 `/code` 볼륨에 있어 컨테이너 안의 root 셸이 고칠 수 있습니다. 하지만 그런 셸은 이미 webmanager 프로세스 환경과 메모리를 읽을 수 있으므로 새로 생기는 구멍은 아닙니다.
- `WEBMANAGER_WEBAUTHN_ENABLED=false`로 끌 수 있습니다.
