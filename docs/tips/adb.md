...
가장 간단한 방법은 [tailscale 연결 문서의 포트 가져오기(forwards)](../tailscale.md#포트-가져오기-forwards)입니다 - 그 방식대로 원하는 기기의 5037 포트(adb 서버)를 가져오면, `ANDROID_ADB_SERVER_ADDRESS=forward` / `ANDROID_ADB_SERVER_PORT=5037` 로 설정하는 것만으로 항상 그 기기의 adb 서버를 쓰게 됩니다.
