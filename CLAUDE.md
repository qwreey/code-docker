...
A browser admin panel (Go backend + Vite/React frontend, `webmanager/` — its own subtree, with its own `CLAUDE.md`/`plan.md`) running alongside code-server as another supervisord program, on port 81. Manages supervisord processes, SSH `authorized_keys`, git config (including commit signing/GPG), tailscale forwards/publish, the vector-backed logs pipeline described above, and an OS-level process/port viewer — see `webmanager/plan.md` for exactly what's implemented vs. still TODO (mise, dind, and a web terminal are the main gaps). No login of its own; relies entirely on the same reverse-proxy forward-auth as code-server, which matters more here than for code-server since some of what it exposes (SSH keys, git credentials) is more sensitive than code-server's own surface.
